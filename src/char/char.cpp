...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if (
		...
		(p->show_equip != cp->show_equip) || (p->hotkey_rowshift2 != cp->hotkey_rowshift2) || (p->inventory_size != cp->inventory_size)
	)
	{	//Save status
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`hotkey_rowshift`='%d', `clan_id`='%d', `title_id`='%lu', `show_equip`='%d', `hotkey_rowshift2`='%d', `inventory_size`='%d'"
			...
			p->hotkey_rowshift, p->clan_id, p->title_id, p->show_equip, p->hotkey_rowshift2, p->inventory_size,
...
int char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count ) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`hotkey_rowshift2`,`inventory_size`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 46, SQLDT_UCHAR,  &p.inventory_size, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`unban_time`,`font`,`uniqueitem_counter`,`sex`,`hotkey_rowshift`,`clan_id`,`title_id`,`show_equip`,`hotkey_rowshift2`,`inventory_size`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 63, SQLDT_UCHAR,  &p->inventory_size, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
