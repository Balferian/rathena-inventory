...
int impossible_trade_check(struct map_session_data *sd)
{
	...
	memcpy(&inventory, &sd->inventory.u.items_inventory, sizeof(struct item) * sd->status.inventory_size);
...
int trade_check(struct map_session_data *sd, struct map_session_data *tsd)
{
	...
	// get inventory of player
	memcpy(&inventory, &sd->inventory.u.items_inventory, sizeof(struct item) * sd->status.inventory_size);
	memcpy(&inventory2, &tsd->inventory.u.items_inventory, sizeof(struct item) * tsd->status.inventory_size);
	...
	for(trade_i = 0; trade_i < 10; trade_i++) {
		...
		if (amount) {
			...
			if (i == sd->status.inventory_size) { // look for an empty slot.
				for(i = 0; i < sd->status.inventory_size && inventory2[i].nameid; i++);
		...
		i = tsd->status.inventory_size;

		if (itemdb_isstackable2(data)) {
			for(i = 0; i < tsd->status.inventory_size; i++)
		...
		if (i == tsd->status.inventory_size) {
			for(i = 0; i < tsd->status.inventory_size && inventory[i].nameid; i++);
			if (i == tsd->status.inventory_size)
				return 0;
